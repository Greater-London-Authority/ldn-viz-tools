<script module lang="ts">
	...
	const root = { name: 'root', children: group(penguins, ['island', 'species']) };
		...
		marks: [
			Plot.rect(leafData, {
				...
				tip: {
					...
					channels: {
						island: { value: (d) => d.parent.data.name, label: '' },
						species: { value: (d) => d.data.name, label: '' },
...
<Story name="Default centred text - two facets">
	{#snippet template()}
		<ObservablePlot
			{spec}
			data={leafData}
