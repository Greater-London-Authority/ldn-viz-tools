...
<label
	class={classNames(
		'flex focus:border-gray-500 focus:border-2 focus:border-dashed',
		disabled ? 'opacity-40' : ''
	)}
	for={inputID}
>
	<svg viewBox="0 0 10 10" height="1.5em" aria-hidden="true">
		<circle cx="5" cy="5" r="4" stroke={color} fill={selectedId === id ? color : 'white'} />

		{#if selectedId === id}
			<circle cx="5" cy="5" r="3" stroke={'white'} fill={'none'} />
		{/if}
	</svg>
	{label}
</label>
