...
<ul
	class="max-height-[60vh] bg-color-input-background text-color-text absolute left-0 top-11 z-40 w-full overflow-y-scroll text-sm shadow-lg"
>
	...
	{#if attribution && attribution.text}
		<li class="text-color-text-muted w-full px-2.5 py-1.5 text-right">
			{#if attribution.link}
				<a href={attribution.link} target="_blank" rel="noopener" class="text-color-text">
					{attribution.text}
				</a>
			{:else}
				{attribution.text}
