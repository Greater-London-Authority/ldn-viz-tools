...
<div class="flex items-center">
	...
	<button
		{...$root}
		use:root
		class="relative h-6 cursor-default rounded-full bg-color-input-background-off transition-colors data-[state=checked]:bg-color-input-background-on w-11 p-0.5"
		id="airplane-mode"
	...
	>
		<span
			class={classNames(
				...
				disabled ? 'bg-color-palette-grey-100' : 'bg-color-input-background'
			)}
