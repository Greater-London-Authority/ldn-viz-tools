...
The `@ldn-viz/ui` package exposes two theme 'modes' within a default theme. Though we tend to use 'theme' and 'mode' interchangeably we actually only have one theme with two modes:
...
The utility functions default to using the current theme, but can take a second argument of a Theme Object in case you need to retrieve a value outside of the currently selected theme
