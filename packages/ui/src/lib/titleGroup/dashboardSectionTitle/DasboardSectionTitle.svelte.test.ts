...
describe('DashboardSectionTitle', () => {
	it('renders the title as an h2 with the section-head role', async () => {
		const { container } = render(DashboardSectionTitle, { title: 'Dashboard section title' });
	...
	it('renders the eyebrow when provided', async () => {
		render(DashboardSectionTitle, { title: 'Dashboard section title', eyebrow: 'Overview' });
	...
	it('applies additional classes passed via the `class` prop', async () => {
		const { container } = render(DashboardSectionTitle, {
			title: 'Dashboard section title',
			class: 'my-class'
		});
