...
export const getThresholdBreaksColorsLabels = ({
	...
	const f = d3.format(labelFormat);
	const labelNumFormat = (val) => val === undefined ? "" : f(val);


	const thresholdObj: {breaks: number[], colors: string[], labels: string[]} = { breaks: [], colors: [], labels: [] };
	...
	const minVal = d3.min(extent) ?? 0;
	const maxVal = d3.max(extent) ?? 0;
