...
describe('colorWithBestContrast', () => {
	...
	describe('background colors from data theme tokens', () => {
		...
		it('chooses white rather than black for secondary background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.secondary, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});

		it('chooses white rather than black for tertiary background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.tertiary, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});

		it('chooses white rather than black for positive background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.positive, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});
		it('chooses white rather than black for negative background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.negative, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});

		it('chooses white rather than black for male background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.male, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});

		it('chooses white rather than black for female background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.female, '#FFFFFF', '#000000')).toBe(
				'#FFFFFF'
			);
		});

		it('chooses black rather than black for empty background', () => {
			expect(colorWithBestContrast(tokens.mode.light.data.empty, '#FFFFFF', '#000000')).toBe(
				'#000000'
			);
		});
