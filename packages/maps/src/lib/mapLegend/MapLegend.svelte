...
{#if isOpen}
	<div
		class="bg-color-container-level-0 pointer-events-auto flex w-full flex-col gap-3 px-4 py-3 sm:max-w-96"
		bind:clientWidth={width}
	>
		<div class="flex justify-between align-middle">
			<p class="text-color-text-primary font-bold">Legend</p>
			<Button
				variant="text"
				size="sm"
				class="bg-color-container-level-0 text-color-text-primary absolute right-1 top-1 cursor-pointer"
				onclick={handleClick}
