...
/**
 * Compute the relative link from the index.md output to the per-component
...
function componentDocLink(filePath: string, packagesDir: string, outputDir: string): string | null {
	...
	const docFile = join(outputDir, 'components', relDir, `${componentName}.md`);
	if (!existsSync(docFile)) return null;
	// Link is relative to the directory containing index.md (which is outputDir)
	return `components/${relDir}/${componentName}.md`;
}
...
function buildComponentDocMap(componentsDir: string, outputDir: string): Map<string, string> {
	...
	const mdFiles = walkDir(componentsDir, (name) => name.endsWith('.md'));
